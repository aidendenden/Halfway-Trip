...
namespace EFPController
{
	...
	public class PlayerMovement : MonoBehaviour
	{
		...
		void Update()
		{
			...
			if (allowCrouch)
			{
				if (crouched && inputManager.GetActionKeyDown(InputManager.Action.Jump))
				{
					crouched = false;
					lastCrouchTime = t;
					OnCrouch?.Invoke(false);
				}
				if (inputManager.GetActionKeyDown(InputManager.Action.Crouch) && !swimming && !climbing)
				{
					if (!crouchState)
					{
			...
			if (grounded)
			{
				if (jumping)
				{
					...
					if (allowJumping)
					{
						...
						if (inputManager.GetActionKeyDown(InputManager.Action.Jump)
						...
						if (!inputManager.GetActionKey(InputManager.Action.Jump) && landStartTime + antiBunnyHopFactor < t)
						{
							jumpBtn = true;
						}
					}
				...
				if (allowJumping && swimming && inputManager.GetActionKeyDown(InputManager.Action.Jump) && !belowWater && canWaterJump)
				{
					jumpTimer = t;
					player.cameraAnimator.SetTrigger(CameraAnimNames.Jump);
					if (jumpfxstate)
			...
			if (climbInteract && !inputManager.GetActionKey(InputManager.Action.Jump)) climbInteract = false;
...
/*#if UNITY_2022_1_OR_NEWER
		...
		void FixedUpdate()
		{
			...
			if (grounded)
			{
				...
				if (allowSprinting)
				{
					...
					{
						if ((inputManager.GetActionKey(InputManager.Action.Sprint) || inputManager.GetActionKeyDown(InputManager.Action.Sprint)))
						{
							if (!sprintStartState)
							{
			...
			if (grounded || climbing)
			{
				...
				if (allowLeaning)
				{
					if (leanDistance > 0f)
					{
						...
						{
							if (inputManager.GetActionKey(InputManager.Action.LeanLeft) && !inputManager.GetActionKey(InputManager.Action.LeanRight)) // lean left
							{
							...
							} else if (inputManager.GetActionKey(InputManager.Action.LeanRight) && !inputManager.GetActionKey(InputManager.Action.LeanLeft)) { // lean right
								// set up horizontal capsule cast to check if player would be leaning into an object
								if (!crouched)
								{
									// move up bottom of lean capsule check when standing to allow player to lean over short objects like boxes 
			...
			if (climbing)
			{
				...
				/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				///
				if (allowDash && !dashState && inputManager.GetActionKey(InputManager.Action.Dash) && grounded && !swimming && !falling && !jumping 
					&& moving)
				{
				...
				if (swimming)
				{
					...
					if (enterWaterTime + 0.2f > t)
					{
						...
						if (!belowWater && !inputManager.GetActionKey(InputManager.Action.Jump))
						{
							canWaterJump = true;
						}
		...
		private void OnTriggerStay(Collider other)
		{
			if (other.gameObject.CompareTag(Game.Tags.Climbable)) // Climbing
			{
				...
				{
					if (!climbInteract && (inputManager.GetActionKey(InputManager.Action.Interact) || inputManager.GetActionKey(InputManager.Action.Jump)))
					{
						climbInteract = true;
						StopClimbing();
					}
