...
{
    [NodeEditor(typeof(PaintMaskNode))]
    public class PaintMaskNodeEditor : ExecutableNodeEditorBase
    {
        ...
        private static readonly GUIContent OPACITY = new GUIContent("Opacity", "Opacity of the brush");

        private static readonly string PAINT_SHADER = "Hidden/Vista/Graph/PaintMaskNode";
        private static readonly int MAIN_TEX = Shader.PropertyToID("_MainTex");
        private static readonly int FALL_OFF = Shader.PropertyToID("_Falloff");
        private static readonly int BRUSH_OPACITY = Shader.PropertyToID("_Opacity");
        ...
        {
            PaintMaskNode n = node as PaintMaskNode;
            ...
            float opacity = EditorGUILayout.Slider(OPACITY, n.brushOpacity, 0f, 1f);
            if (EditorGUI.EndChangeCheck())
            {
                m_graphEditor.RegisterUndo(n);
                n.brushRadius = radius;
                n.brushFalloff = falloff;
                n.brushOpacity = opacity;
            }
        }
        ...
        {
            PaintMaskNode n = node as PaintMaskNode;
            //EditorGUI.DrawPreviewTexture(imageRect, n.texture);
            Handles.BeginGUI();
            ...
            if (Event.current.isMouse && Event.current.button == 0)
            {
                ...
                paintMaterial.SetFloat(BRUSH_OPACITY, n.brushOpacity);

                Vector2[] quad = CalculateQuad(imageRect, Event.current.mousePosition, n.brushRadius);

                RenderTexture tempRT = RenderTexture.GetTemporary(n.resolution, n.resolution, 0, RenderTextureFormat.RFloat, RenderTextureReadWrite.Linear);
