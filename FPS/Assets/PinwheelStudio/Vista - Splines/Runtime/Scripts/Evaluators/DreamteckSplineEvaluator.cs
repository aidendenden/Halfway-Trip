...
namespace Pinwheel.Vista.Splines
{
    ...
    public class DreamteckSplineEvaluator : SplineEvaluatorComponentBase, ITrianglesBufferProvider
    {
        [SerializeField]
        ...
        protected override void OnEnable()
        {
            ...
            m_spline.onRebuild += ClearSplineCache;
        }
        ...
        {
            SplinePoint[] splinePoints = m_spline.GetPoints(SplineComputer.Space.World);
            Vector3[] worldPoints = new Vector3[splinePoints.Length];
            for (int i = 0; i < splinePoints.Length; ++i)
            {
                worldPoints[i] = splinePoints[i].position;
            }
        ...
        private void ExtractWorldPoints()
        {
            ...
            {
                Vector3 wp = m_spline.EvaluatePosition(i * step, SplineComputer.EvaluateMode.Calculate);
        ...
        private void ExtractVerticesAndAlphas()
        {
            ...
            for (int i = 0; i < segmentCount; ++i)
            {
                ...
                m_spline.Evaluate(t0, ref s, SplineComputer.EvaluateMode.Calculate);
                c0 = s.position;
                normal0 = Vector3.Cross(s.forward, s.up).normalized;

                m_spline.Evaluate(t1, ref s, SplineComputer.EvaluateMode.Calculate);
                c1 = s.position;
                normal1 = Vector3.Cross(s.forward, s.up).normalized;
