...
namespace Pinwheel.Vista.Splines
{
    ...
    public class UnitySplineEvaluator : SplineEvaluatorComponentBase, ITrianglesBufferProvider
    {
        [SerializeField]
        ...
        protected override void OnEnable()
        {
            ...
            m_spline.Spline.changed += ClearSplineCache;
        }
        ...
        {
            Vector3[] worldPoints = new Vector3[m_spline.Spline.Count];
            int i = 0;
            foreach (BezierKnot knot in m_spline.Spline.Knots)
            {
                Vector3 wp = transform.TransformPoint(knot.Position);
                worldPoints[i] = wp;
                i += 1;
            }
        ...
        private void ExtractWorldPoints()
        {
            ...
            {
                Vector3 wp = m_spline.EvaluatePosition(i * step);
        ...
        private void ExtractVerticesAndAlphas()
        {
            ...
            float3 c0, up0, tangent0, normal0;
            float3 c1, up1, tangent1, normal1;
            float3 l0, l1, r0, r1;
            ...
            for (int i = 0; i < segmentCount; ++i)
            {
                ...
                m_spline.Evaluate(t0, out c0, out tangent0, out up0);
                m_spline.Evaluate(t1, out c1, out tangent1, out up1);
