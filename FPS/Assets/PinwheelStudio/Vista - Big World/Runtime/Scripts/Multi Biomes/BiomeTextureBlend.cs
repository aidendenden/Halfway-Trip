...
namespace Pinwheel.Vista.BigWorld
{
    internal static class BiomeTextureBlend
    {
        ...
        internal static void BlendTextureWeights(BiomeData destData, List<BiomeData> srcDatas)
        {
            ...
            List<Texture> blendedMasks = srcMasks;

            res = int.MinValue;
            format = RenderTextureFormat.RFloat;
            List<TerrainLayer> srcLayers = new List<TerrainLayer>();
            List<RenderTexture> srcWeights = new List<RenderTexture>();
            List<Texture> maskByLayer = new List<Texture>();
            for (int b = 0; b < srcDatas.Count; ++b)
            {
                BiomeData srcData = srcDatas[b];
                ...
                {
                    maskByLayer.Add(blendedMasks[b]);
                }
            }
            ...
            WeightsBlend.Blend(destWeights.ToArray(), srcWeights.ToArray(), maskByLayer.ToArray());
        ...
        private static void Blend(ComputeShader blendShader, RenderTexture src, RenderTexture dest, RenderTexture biomeMask, BiomeBlendOptions.TextureBlendMode blendMode)
        {
            if (dest == null)
            {
            ...
            blendShader.SetTexture(kernel, BIOME_MASK_TEXTURE, biomeMask != null ? (Texture)biomeMask : Texture2D.blackTexture);
