...
namespace Whilefun.FPEKit
{
    ...
    {
        [Header("User Input options")] [SerializeField, Tooltip("If true, Only Mouse Y Axis will be flipped for Look")]
        ...
        [SerializeField,
         Tooltip("If true, player look movement sensitivity will be boosted up (by gamepadLookSensitivityBoost)")]
        ...
        [SerializeField,
         Tooltip(
             "If smoothing is on, this factor determines how quickly the mouse slows down. Higher value means snappier look movements.")]
        private float lookSmoothFactor = 10.0f;

        public float LookSmoothFactor
        {
            get { return lookSmoothFactor; }
        }

        [SerializeField,
         Tooltip(
             "The absolute value the right stick must meet or exceed before a virtual 'right' button press is captured.")]
        private float rightStickButtonThreshold = 0.9f;

        [SerializeField,
         Tooltip(
             "The number of seconds before a right stick 'button press' will be repeated if the stick is held left or right in position.")]
        private float rightStickButtonRepeatTime = 0.2f;

        private float rightStickButtonRepeatCounter = 0.0f;

        [SerializeField,
         Tooltip("Deadzone for game pad analog sticks. Any value less than this will be considered 'zero'")]
        ...
        void Awake()
        {
            ...
            FPEVirtualButtons.Add(eFPEInput.FPE_INPUT_INTERACT,
                new FPEInputButton(eFPEInput.FPE_INPUT_INTERACT, "Interact"));
            FPEVirtualButtons.Add(eFPEInput.FPE_INPUT_EXAMINE,
                new FPEInputButton(eFPEInput.FPE_INPUT_EXAMINE, "Examine"));
            ...
            FPEVirtualButtons.Add(eFPEInput.FPE_INPUT_PUT_IN_INVENTORY,
                new FPEInputButton(eFPEInput.FPE_INPUT_PUT_IN_INVENTORY, "Put In Inventory"));
            FPEVirtualButtons.Add(eFPEInput.FPE_INPUT_MENU, new FPEInputButton(eFPEInput.FPE_INPUT_MENU, "Menu"));
            FPEVirtualButtons.Add(eFPEInput.FPE_INPUT_MENU_PREVIOUS_TAB,
                new FPEInputButton(eFPEInput.FPE_INPUT_MENU_PREVIOUS_TAB, "Menu Previous Tab"));
            FPEVirtualButtons.Add(eFPEInput.FPE_INPUT_MENU_NEXT_TAB,
                new FPEInputButton(eFPEInput.FPE_INPUT_MENU_NEXT_TAB, "Menu Next Tab"));
            FPEVirtualButtons.Add(eFPEInput.FPE_INPUT_MENU_PREVIOUS_PAGE,
                new FPEInputButton(eFPEInput.FPE_INPUT_MENU_PREVIOUS_PAGE, "Menu Previous Page"));
            FPEVirtualButtons.Add(eFPEInput.FPE_INPUT_MENU_NEXT_PAGE,
                new FPEInputButton(eFPEInput.FPE_INPUT_MENU_NEXT_PAGE, "Menu Next Page"));
            ...
            FPEVirtualButtons.Add(eFPEInput.FPE_INPUT_HORIZONTAL,
                new FPEInputButton(eFPEInput.FPE_INPUT_HORIZONTAL, "Horizontal Button"));
            FPEVirtualButtons.Add(eFPEInput.FPE_INPUT_VERTICAL,
                new FPEInputButton(eFPEInput.FPE_INPUT_VERTICAL, "Vertical Button"));

            // Add all the virtual axes you want to poll later from other scripts
            FPEVirtualAxes.Add(eFPEInput.FPE_INPUT_MOUSELOOKX,
                new FPEInputAxis(eFPEInput.FPE_INPUT_MOUSELOOKX, "Mouse Look X"));
            FPEVirtualAxes.Add(eFPEInput.FPE_INPUT_MOUSELOOKY,
                new FPEInputAxis(eFPEInput.FPE_INPUT_MOUSELOOKY, "Mouse Look Y"));
            ...
            FPEVirtualAxes.Add(eFPEInput.FPE_INPUT_HORIZONTAL,
                new FPEInputAxis(eFPEInput.FPE_INPUT_HORIZONTAL, "Horizontal"));
            FPEVirtualAxes.Add(eFPEInput.FPE_INPUT_VERTICAL,
                new FPEInputAxis(eFPEInput.FPE_INPUT_VERTICAL, "Vertical"));
        ...
        void Update()
        {
            ...
            if ((Input.GetAxis("Gamepad Look X") * lookSensitivity.x * gamepadAnalogStickSensitivityMultplier) <
                -rightStickButtonThreshold && rightStickButtonRepeatCounter <= 0.0f)
            {
                FPEVirtualButtons[eFPEInput.FPE_INPUT_MENU_PREVIOUS_PAGE].Pressed();
                rightStickButtonRepeatCounter = rightStickButtonRepeatTime;
            }

            if ((Input.GetAxis("Gamepad Look X") * lookSensitivity.x * gamepadAnalogStickSensitivityMultplier) < 0 &&
                (Input.GetAxis("Gamepad Look X") * lookSensitivity.x * gamepadAnalogStickSensitivityMultplier) >
                rightStickButtonThreshold)
            {
                FPEVirtualButtons[eFPEInput.FPE_INPUT_MENU_PREVIOUS_PAGE].Released();
            ...
            if (Input.GetButton("Run") || (Input.GetAxis("Gamepad Run") > triggerDeadzone &&
                                           previousLeftTriggerValue <= triggerDeadzone))
            {
            ...
            if ((!Input.GetButton("Run") && Input.GetAxis("Gamepad Run") <= triggerDeadzone) ||
                (Input.GetAxis("Gamepad Run") <= triggerDeadzone && previousLeftTriggerValue > triggerDeadzone))
            {
            ...
            {
                FPEVirtualAxes[eFPEInput.FPE_INPUT_MOUSELOOKY]
                    .Update(Input.GetAxis("Mouse Y") * lookSensitivity.y * -1.0f);
            }
            else
            {
            ...
            {
                FPEVirtualAxes[eFPEInput.FPE_INPUT_LOOKX].Update(Input.GetAxisRaw("Gamepad Look X") *
                                                                 lookSensitivity.x *
                                                                 gamepadAnalogStickSensitivityMultplier *
                                                                 appliedGamepadBoost);
            }
            else
            {
            ...
            if (Mathf.Abs(Input.GetAxisRaw("Gamepad Look Y")) > analogStickDeadzone)
            {
                ...
                {
                    FPEVirtualAxes[eFPEInput.FPE_INPUT_LOOKY].Update(Input.GetAxisRaw("Gamepad Look Y") *
                                                                     lookSensitivity.y *
                                                                     gamepadAnalogStickSensitivityMultplier *
                                                                     appliedGamepadBoost);
                }
        ...
        public bool GetButton(eFPEInput buttonID)
        {
            ...
            {
                Debug.LogError("FPEInputManager.GetButtonDown:: No button ID '" + buttonID +
                               "'. Are you looking for an axis instead?");
                return false;
            }
        }
        ...
        public float GetAxis(eFPEInput axisID)
        {
            ...
            {
                Debug.LogError("FPEInputManager.GetAxis:: No axis ID '" + axisID +
                               "'. Are you looking for a button instead?");
                return 0.0f;
            }
        }
