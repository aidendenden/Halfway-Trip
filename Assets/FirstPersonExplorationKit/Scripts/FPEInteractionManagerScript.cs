...
namespace Whilefun.FPEKit
{
    ...
    public class FPEInteractionManagerScript : MonoBehaviour
    {
        ...
        [Tooltip(
            "When examining an object, it will only rotate if axis input is greater than this deadzone value. Default is 0.1.")]
        public float examinationDeadzone = 0.1f;

        [Tooltip(
            "When examining an object, this value acts as a multiplier to Input Mouse X/Y values. Default is 300.")]
        ...
        [Header("Special Interaction Masks")] [Tooltip("This should be assigned to FPEPutbackObjects layer")]
        public LayerMask putbackLayerMask;

        [Tooltip(
            "This should be assigned to be mixed to include everything except the FPEPutbackObjects and FPEIgnore layers")]
        ...
        [Tooltip(
            "This is the FOV the camera will use when player Right-Clicks to zoom in. Un-zoomed FOV is set to be that of Main Camera when scene starts. If you change FOV in Main Camera, also change it in ExaminationCamera.")]
        ...
        private FPEFirstPersonController.ePlayerDockingState currentDockActionType =
            FPEFirstPersonController.ePlayerDockingState.IDLE;
        ...
        {
            get { return playerSuspendedForSaveLoad; }
        }
...
#if UNITY_EDITOR
        [Header("Debug")] [SerializeField, Tooltip("If true, debug gizmos will be drawn (Editor Only)")]
        ...
        void Awake()
        {
            ...
            {
                Debug.LogWarning(
                    "FPEInteractionManager:: Duplicate instance of FPEInteractionManager, deleting second one.");
        ...
        public void initialize()
        {
            if (!initialized)
            {
                ...
                if (!inventoryManager)
                {
                    Debug.LogError("FPEInteractionManager:: There was no FPEInventoryManagerScript attached to '" +
                                   gameObject.name + "'. Did you change the FPEInteractionManager prefab?");
                }
                ...
                interactionObjectPickupLocation =
                    thePlayer.transform.Find("MainCamera/ObjectPickupLocation").gameObject;
                interactionObjectExamineLocation =
                    thePlayer.transform.Find("MainCamera/ObjectExamineLocation").gameObject;
                interactionObjectTossLocation = thePlayer.transform.Find("MainCamera/ObjectTossLocation").gameObject;
                interactionInventoryLocation =
                    thePlayer.transform.Find("MainCamera/ObjectInInventoryPosition").gameObject;
                audioDiaryPlayer = thePlayer.transform.Find("FPEAudioDiaryPlayer").gameObject;
                secondaryInteractionSoundPlayer =
                    thePlayer.transform.Find("FPESecondaryInteractionSoundPlayer").gameObject;
                ...
                if (!interactionObjectPickupLocation || !interactionObjectExamineLocation ||
                    !interactionObjectTossLocation || !interactionInventoryLocation)
                {
                    Debug.LogError(
                        "FPEInteractionManagerScript:: Player or its components are missing. Did you change the Player Controller prefab, or forget to tag with 'Player' tag?");
                }

                if (!audioDiaryPlayer || !secondaryInteractionSoundPlayer || !journalSFXPlayer || !genericSFXPlayer)
                {
                    Debug.LogError(
                        "FPEInteractionManagerScript:: FPEAudioDiaryPlayer, FPESecondaryInteractionSoundPlayer, or FPEJournalSFX are missing from Player Controller. Did you break the FPEPlayerController prefab or forget to add one or both of these prefabs to your player controller?");
                }
                ...
                {
                    Debug.LogWarning(
                        "FPEInteractionManagerScript:: There is no FPEMenu present in your scene. This will mean openMenu() and closeMenu() won't function as expected but the game will still run.");
                }
        ...
        void Update()
        {
            ...
            else if (currentInteractionState == eInteractionState.FREE)
            {
                ...
                if (!examiningObject && !dockingInProgress && hitInteractable.transform != null &&
                    hitInteractable.transform.gameObject.GetComponent<FPEInteractableBaseScript>())
                {
                    if (hitInteractable.distance < hitInteractable.transform.gameObject
                            .GetComponent<FPEInteractableBaseScript>().getInteractionDistance())
                    {
                        ...
                        if (currentInteractableObject.GetComponent<FPEInteractableBaseScript>().getInteractionType() ==
                            FPEInteractableBaseScript.eInteractionType.STATIC)
                        {
                ...
                else
                {
                    ...
                    if (!examiningObject && hitPutBack2.transform != null &&
                        hitPutBack2.transform.gameObject.GetComponent<FPEPutBackScript>() && currentHeldObject != null)
                    {
                        if (hitPutBack2.transform.gameObject.GetComponent<FPEPutBackScript>()
                                .putBackMatchesGameObject(currentHeldObject) && (hitPutBack2.distance <
                                hitPutBack2.transform.gameObject.GetComponent<FPEPutBackScript>()
                ...
                if (FPEInputManager.Instance.GetButtonDown(FPEInputManager.eFPEInput.FPE_INPUT_INTERACT) &&
                    !examiningObject && !dockingInProgress)
                {
                    ...
                    if (currentHeldObject)
                    {
                        ...
                        if (currentPutbackObject)
                        {
                            ...
                            currentHeldObject.GetComponent<Rigidbody>().collisionDetectionMode =
                        ...
                        else if (currentInteractableObject && currentInteractableObject
                                     .GetComponent<FPEInteractableBaseScript>()
                                     .interactionsAllowedWhenHoldingObject() == true)
                        {
                            if (currentInteractableObject.GetComponent<FPEInteractableBaseScript>()
                                    .getInteractionType() == FPEInteractableBaseScript.eInteractionType.STATIC)
                            {
                                retrieveNote(currentInteractableObject);
                            ...
                            else if (currentInteractableObject.GetComponent<FPEInteractableBaseScript>()
                                         .getInteractionType() == FPEInteractableBaseScript.eInteractionType.ACTIVATE)
                            {
                                retrieveNote(currentInteractableObject);
                                retrievePassiveAudioDiary(currentInteractableObject, true);
                                attemptActivation(currentInteractableObject);
                                         ...
                                         .getInteractionType() == FPEInteractableBaseScript.eInteractionType.PICKUP)
                            {
                                ...
                                retrievePassiveAudioDiary(currentInteractableObject, true);
                                         ...
                                         .getInteractionType() == FPEInteractableBaseScript.eInteractionType.JOURNAL)
                            {
                                // Journals always require two hands
                                retrievePassiveAudioDiary(currentInteractableObject, true);
                            }
                            else if (currentInteractableObject.GetComponent<FPEInteractableBaseScript>()
                                         .getInteractionType() == FPEInteractableBaseScript.eInteractionType.INVENTORY)
                            {
                                // See "Put In Inventory" section below
                            }
                            else if (currentInteractableObject.GetComponent<FPEInteractableBaseScript>()
                                         .getInteractionType() == FPEInteractableBaseScript.eInteractionType.DOCK)
                            {
                                if (currentInteractableObject.GetComponent<FPEInteractableDockScript>().isOccupied() ==
                                    false)
                                {
                                    ...
                                    retrievePassiveAudioDiary(currentInteractableObject, true);
                                         ...
                                         .getInteractionType() == FPEInteractableBaseScript.eInteractionType.AUDIODIARY)
                            {
                                        ...
                                        .AutomaticPlayback == false)
                                {
                                    ...
                                    currentInteractableObject.GetComponent<FPEInteractableAudioDiaryScript>()
                                        .interact();
                                }
                            }
                            /*
                            ...
                            */
                            ...
                            {
                                Debug.LogWarning("FPEInteractionManagerScript:: Unhandled eInteractionType '" +
                                                 currentInteractableObject.GetComponent<FPEInteractableBaseScript>()
                                                     .getInteractionType() +
                                                 "' for object that allows interaction when object being held. No case exists to manage this.");
                            }
                        ...
                        // If I am already holding something, and NOT allowed to interact with the currently highlighted thing while there's something in my hand
                                     ...
                                     .GetComponent<FPEInteractableBaseScript>()
                                     .interactionsAllowedWhenHoldingObject() == false)
                        {
                            if (currentInteractableObject.GetComponent<FPEInteractableBaseScript>()
                                    .getInteractionType() == FPEInteractableBaseScript.eInteractionType.STATIC)
                            {
                            ...
                            */
                            else
                            {
                                Debug.LogWarning("FPEInteractionManagerScript:: Unhandled eInteractionType '" +
                                                 currentInteractableObject.GetComponent<FPEInteractableBaseScript>()
                                                     .getInteractionType() +
                                                 "' for object that DOES NOT allow interaction when object being held. No case exists to manage this.");
                            }
                        }
                    ...
                    else
                    {
                        ...
                        if (currentInteractableObject)
                        {
                                    ...
                                    .getInteractionType() == FPEInteractableBaseScript.eInteractionType.PICKUP)
                            {
                                ...
                                currentInteractableObject.GetComponent<FPEInteractablePickupScript>()
                                    .doPickupPutdown(false);
                                currentInteractableObject.GetComponent<FPEInteractableBaseScript>().unHighlightObject();
                                         ...
                                         .getInteractionType() == FPEInteractableBaseScript.eInteractionType.STATIC)
                            {
                                currentInteractableObject.GetComponent<FPEInteractableBaseScript>().interact();
                                         ...
                                         .getInteractionType() == FPEInteractableBaseScript.eInteractionType.JOURNAL)
                            {
                                currentInteractableObject.GetComponent<FPEInteractableJournalScript>()
                                    .activateJournal();
                                         ...
                                         .getInteractionType() == FPEInteractableBaseScript.eInteractionType.INVENTORY)
                            {
                                    ...
                                    .PickupPermitted)
                                {
                                    ...
                                    currentInteractableObject.GetComponent<FPEInteractableInventoryItemScript>()
                                        .doPickupPutdown(false);
                                    currentInteractableObject.GetComponent<FPEInteractableBaseScript>()
                                        .unHighlightObject();
                                    moveObjectToPlayersHand(currentInteractableObject);
                            ...
                            */
                            else
                            {
                                Debug.LogWarning("FPEInteractionManagerScript:: Unhandled eInteractionType '" +
                                                 currentInteractableObject.GetComponent<FPEInteractableBaseScript>()
                                                     .getInteractionType() +
                                                 "'. No case exists to manage this (Which might be just fine, depending).");
                            }
                ...
                // Examine held object //
                if (FPEInputManager.Instance.GetButtonDown(FPEInputManager.eFPEInput.FPE_INPUT_EXAMINE) &&
                    currentHeldObject && !dockingInProgress)
                {
                    ...
                    if (currentHeldObject.GetComponent<FPEInteractablePickupScript>()
                            .postExaminationInteractionString != "")
                    {
                        currentHeldObject.GetComponent<FPEInteractablePickupScript>().interactionString =
                            currentHeldObject.GetComponent<FPEInteractablePickupScript>()
                                .postExaminationInteractionString;
                    }

                    currentHeldObject.transform.position = interactionObjectExamineLocation.transform.position;

                    if (currentHeldObject.GetComponent<FPEInteractablePickupScript>().rotationLockType ==
                        FPEInteractablePickupScript.eRotationType.FREE)
                    {
                        if (lastObjectHeldRotation == Quaternion.identity)
                        {
                            ...
                            currentHeldObject.transform.Rotate(currentHeldObject
                                .GetComponent<FPEInteractablePickupScript>().pickupRotationOffset);
                        }
                        else
                        {
                ...
                if (FPEInputManager.Instance.GetButtonUp(FPEInputManager.eFPEInput.FPE_INPUT_EXAMINE) &&
                    currentHeldObject)
                {
                ...
                if (currentHeldObject)
                {
                    ...
                    if (examiningObject)
                    {
                        ...
                        float examinationOffsetUp = currentHeldObject.GetComponent<FPEInteractablePickupScript>()
                            .examinationOffsetUp;
                        float examinationOffsetForward = currentHeldObject.GetComponent<FPEInteractablePickupScript>()
                        ...
                        float examinationChangeX =
                            FPEInputManager.Instance.GetAxis(FPEInputManager.eFPEInput.FPE_INPUT_MOUSELOOKX);
                        float examinationChangeY =
                            FPEInputManager.Instance.GetAxis(FPEInputManager.eFPEInput.FPE_INPUT_MOUSELOOKY);
                        ...
                        if (examinationChangeX == 0 & examinationChangeY == 0)
                        {
                            ...
                            examinationChangeX =
                                FPEInputManager.Instance.GetAxis(FPEInputManager.eFPEInput.FPE_INPUT_LOOKX) *
                                Time.deltaTime;
                            examinationChangeY =
                                FPEInputManager.Instance.GetAxis(FPEInputManager.eFPEInput.FPE_INPUT_LOOKY) *
                                Time.deltaTime;
                        }
                    ...
                    else
                    {
                        ...
                        currentHeldObject.transform.rotation = Quaternion.Slerp(currentHeldObject.transform.rotation,
                            interactionObjectPickupLocation.transform.rotation * Quaternion.Euler(
                                lastObjectHeldRotation.eulerAngles + currentHeldObject
                                    .GetComponent<FPEInteractablePickupScript>().pickupRotationOffset), 0.2f);
                ...
                if (FPEInputManager.Instance.GetButtonDown(FPEInputManager.eFPEInput.FPE_INPUT_PUT_IN_INVENTORY) &&
                    !dockingInProgress)
                {
                    // Already holding an object //
                    if (currentHeldObject)
                        ...
                        if (!examiningObject)
                        {
                            ...
                            if (currentInteractableObject)
                            {
                                if (currentInteractableObject.GetComponent<FPEInteractableBaseScript>()
                                        .getInteractionType() == FPEInteractableBaseScript.eInteractionType.INVENTORY)
                                {
                                    putObjectIntoInventory(currentInteractableObject
                                        .GetComponent<FPEInteractableInventoryItemScript>());
                                }
                                else
                                {
                                    putObjectIntoInventory(currentHeldObject
                                        .GetComponent<FPEInteractableInventoryItemScript>());
                                    currentHeldObject = null;
                                }
                            }
                            ...
                            {
                                if (currentHeldObject.GetComponent<FPEInteractableBaseScript>().getInteractionType() ==
                                    FPEInteractableBaseScript.eInteractionType.INVENTORY)
                                {
                                    putObjectIntoInventory(currentHeldObject
                                        .GetComponent<FPEInteractableInventoryItemScript>());
                ...
                if (dockingInProgress)
                {
                    ...
                    if (dockingCompleted())
                    {
                        ...
                        {
                            Debug.LogWarning(
                                "FPEInteractionManagerScript:: Dock action finished, but was not dock or undock, which makes no sense. Did you start a dock using alternate functions?");
                        }
                ...
                if (cameraZoomedIn)
                {
                    Camera.main.fieldOfView = Mathf.Lerp(Camera.main.fieldOfView, zoomedFOV,
                        cameraZoomFOVChangePerSecond * Time.deltaTime);
                }
                else
                {
                    Camera.main.fieldOfView = Mathf.Lerp(Camera.main.fieldOfView, unZoomedFOV,
                        cameraZoomFOVChangePerSecond * Time.deltaTime);
                }
                ...
                {
                    FPEInputManager.Instance.LookSensitivity = Vector2.MoveTowards(
                        FPEInputManager.Instance.LookSensitivity,
                        new Vector2(targetMouseSensitivity.x, targetMouseSensitivity.y),
                        smoothMouseChangeRate * Time.deltaTime);
                }
            }
        ...
        public void playNewAudioDiary(GameObject diary)
        {
            ...
            myHUD.StartPlayingNewAudioDiary(diary.GetComponent<FPEInteractableAudioDiaryScript>().audioDiaryTitle,
                false);
            fadingDiaryAudio = false;
            audioDiaryPlayer.GetComponent<AudioSource>().clip =
                diary.GetComponent<FPEInteractableAudioDiaryScript>().audioDiaryClip;
            ...
            {
                inventoryManager.addAudioDiaryEntry(new FPEAudioDiaryEntry(
                    diary.GetComponent<FPEInteractableAudioDiaryScript>().audioDiaryTitle,
                    diary.GetComponent<FPEInteractableAudioDiaryScript>().audioDiaryClip,
                    diary.GetComponent<FPEInteractableAudioDiaryScript>().ShowDiaryTitle));
            }
        }
        ...
        public void playSecondaryInteractionAudio(AudioClip secondaryClip, bool playAsAudioDiary,
            string audioLogText = "")
        {
            // if currently playing an audio diary, stop current one, reset text for new one
            if (playingAudioDiary)
        ...
        public void openJournal()
        {
            ...
            {
                Debug.LogError("Journal '" + currentJournal.name +
                               "' opened, but was assigned no pages. Assign Sprites to journalPages array in the Inspector.");
            }
        }
        ...
        {
            FPEInteractableBaseScript.eInteractionType currentHeldType =
                FPEInteractionManagerScript.Instance.getHeldObjectType();
            ...
            {
                Debug.Log("FPEInteractionManagerScript.holdObjectFromInventory():: Asked to hold unsupported type '" +
                          currentHeldType + "' object. Nothing will be held.");
            }
        }
        ...
        public void consumeObjectFromInventory(FPEInteractableInventoryItemScript itemToConsume)
        {
            ...
            {
                Debug.LogError(
                    "FPEInteractionManagerScript.consumeObjectFromInventory() asked to consume item '" +
                    itemToConsume.gameObject.name +
                    "' but this item has no FPEInventoryConsumer attached. Destroying item instead!",
                    itemToConsume.gameObject);
            }
        }
        ...
        public void tossObject(FPEInteractablePickupScript pickup)
        {
            ...
            go.transform.position = interactionObjectTossLocation.transform.position + Vector3.up * tossOffsetUp +
                                    Camera.main.transform.forward * tossOffsetForward;
            go.GetComponent<Rigidbody>().AddForce(Camera.main.transform.forward * tossImpulseFactor * tossStrength,
                ForceMode.Impulse);
        ...
        public FPEInteractableBaseScript.eInteractionType getHeldObjectType()
        {
            return ((currentHeldObject == null)
                ? FPEInteractableBaseScript.eInteractionType.NULL_TYPE
                : currentHeldObject.GetComponent<FPEInteractableBaseScript>().getInteractionType());
        }
        ...
        private void retrieveNote(GameObject interactableObject)
        {
            ...
            {
                showNotification("New note '" +
                                 interactableObject.gameObject.GetComponent<FPEAttachedNote>().NoteTitle + "' added");
                genericSFXPlayer.GetComponent<AudioSource>().PlayOneShot(noteAdded);
                inventoryManager.addNoteEntry(interactableObject.gameObject.GetComponent<FPEAttachedNote>()
                    .collectNote());
            }
        }
        ...
        private void retrievePassiveAudioDiary(GameObject interactableObject, bool wasInteractedWith)
        {
            if (interactableObject.gameObject.GetComponent<FPEPassiveAudioDiary>())
            {
                ...
                if ((wasInteractedWith == true) || ((wasInteractedWith == false) &&
                                                    (interactableObject.gameObject.GetComponent<FPEPassiveAudioDiary>()
                                                        .AutomaticPlayback == true)))
                {
                    if (interactableObject.gameObject.GetComponent<FPEPassiveAudioDiary>().HasBeenPlayed == false)
                    {
                        ...
                        FPEAudioDiaryEntry tempEntry = new FPEAudioDiaryEntry(
                            interactableObject.gameObject.GetComponent<FPEPassiveAudioDiary>().DiaryTitle,
                            interactableObject.gameObject.GetComponent<FPEPassiveAudioDiary>().DiaryAudio,
                            interactableObject.gameObject.GetComponent<FPEPassiveAudioDiary>().ShowDiaryTitle);
                        playAudioDiaryEntry(tempEntry, false);

                        if (interactableObject.gameObject.GetComponent<FPEPassiveAudioDiary>().AddEntryToInventory ==
                            true)
                        {
        ...
        private void attemptActivation(GameObject objectToActivate)
        {
            ...
            if (!activatee)
            {
                Debug.Log("FPEInteractionManagerScript.attemptActivation():: Given non-Activate type object called '" +
                          objectToActivate.name + "'. No activation will take place.");
            }
            else
            {
                ...
                if ((activatee.RequireInventoryItem) ||
                    (activatee.EventFireType == FPEGenericEvent.eEventFireType.TOGGLE && activatee.IsToggledOn &&
                     activatee.ToggleOffRequiresInventory))
                {
                    // Required in hand
                    if (activatee.RequiredToBeInHand == FPEInteractableActivateScript.eInventoryRequirementMode.IN_HAND)
                    {
                        if ((currentHeldObject != null &&
                             currentHeldObject.GetComponent<FPEInteractableInventoryItemScript>() &&
                             currentHeldObject.GetComponent<FPEInteractableInventoryItemScript>().InventoryItemType ==
                             activatee.RequiredItemType))
                        {
                    ...
                    else if (activatee.RequiredToBeInHand ==
                             FPEInteractableActivateScript.eInventoryRequirementMode.IN_INVENTORY)
                    {
                        if (inventoryManager.inventoryQuantity(activatee.RequiredItemType) >=
                            activatee.RequiredInventoryQuantity)
                        {
                            if (activatee.RemoveOnUse)
                            {
                                inventoryManager.destroyInventoryItemsOfType(activatee.RequiredItemType,
                                    activatee.RequiredInventoryQuantity);
                            }
                    ...
                    else
                    {
                        ...
                        else if (inventoryManager.inventoryQuantity(activatee.RequiredItemType) >=
                                 activatee.RequiredInventoryQuantity)
                        {
                            if (activatee.RemoveOnUse)
                            {
        ...
        {
            FPEInteractableBaseScript.eInteractionType heldType = (currentHeldObject != null)
                ? currentHeldObject.GetComponent<FPEInteractableBaseScript>().getInteractionType()
                : FPEInteractableBaseScript.eInteractionType.NULL_TYPE;
            FPEInteractableBaseScript.eInteractionType lookedAtType = (currentInteractableObject != null)
                ? currentInteractableObject.GetComponent<FPEInteractableBaseScript>().getInteractionType()
                : FPEInteractableBaseScript.eInteractionType.NULL_TYPE;
                ...
                switch (lookedAtType)
                {
                    ...
                    case FPEInteractableBaseScript.eInteractionType.ACTIVATE:
                        if (currentInteractableObject.GetComponent<FPEInteractableActivateScript>()
                            .interactionsAllowedWhenHoldingObject())
                        {
                            currentInteractableObject.GetComponent<FPEInteractableBaseScript>().highlightObject();
                        }
            ...
            {
                Debug.LogError("FPEInteractionManagerScript.updateControlHints():: Player is holding object of type '" +
                               heldType + "' which is not implemented.");
            }
        }
        ...
        private void refreshHUDData()
        {
            ...
            // Held/LookedAt Types
            myHUDData.heldType = (currentHeldObject != null)
                ? currentHeldObject.GetComponent<FPEInteractableBaseScript>().getInteractionType()
                : FPEInteractableBaseScript.eInteractionType.NULL_TYPE;
            myHUDData.lookedAtType = (currentInteractableObject != null)
                ? currentInteractableObject.GetComponent<FPEInteractableBaseScript>().getInteractionType()
                : FPEInteractableBaseScript.eInteractionType.NULL_TYPE;
            ...
            if (currentHeldObject != null)
            {
                myHUDData.heldObjectInteractionString =
                    currentHeldObject.GetComponent<FPEInteractablePickupScript>().interactionString;
                myHUDData.heldObjectinteractionsAllowedWhenHoldingObject = currentHeldObject
                    .GetComponent<FPEInteractablePickupScript>().interactionsAllowedWhenHoldingObject();

                if (currentHeldObject.GetComponent<FPEInteractableInventoryItemScript>())
                {
                    myHUDData.heldObjectInventoryItemName =
                        currentHeldObject.GetComponent<FPEInteractableInventoryItemScript>().ItemName;
                }
            }
            else
            {
            ...
            {
                myHUDData.lookedAtInteractionString = currentInteractableObject
                    .GetComponent<FPEInteractableBaseScript>().interactionString;

                if (currentInteractableObject.GetComponent<FPEInteractableInventoryItemScript>())
                {
                    myHUDData.lookedAtInventoryPickupPermitted = currentInteractableObject
                        .GetComponent<FPEInteractableInventoryItemScript>().PickupPermitted;
                    myHUDData.lookedAtInventoryItemName = currentInteractableObject
                        .GetComponent<FPEInteractableInventoryItemScript>().ItemName;
                }

                if (currentInteractableObject.GetComponent<FPEInteractableDockScript>())
                {
                    myHUDData.lookedAtDockHint = currentInteractableObject.GetComponent<FPEInteractableDockScript>()
                        .DockHintText;
                    myHUDData.lookedAtDockOccupied =
                        currentInteractableObject.GetComponent<FPEInteractableDockScript>().Occupied;
                }

                if (currentInteractableObject.GetComponent<FPEInteractablePickupScript>())
                {
                    myHUDData.lookedAtPickupInteractionsAllowedWhenHoldingObject = currentInteractableObject
                        .GetComponent<FPEInteractablePickupScript>().interactionsAllowedWhenHoldingObject();
                }

                if (currentInteractableObject.GetComponent<FPEInteractableActivateScript>())
                {
                    myHUDData.lookedAtActivateAllowedWhenHoldingObject = currentInteractableObject
                        .GetComponent<FPEInteractableActivateScript>().interactionsAllowedWhenHoldingObject();
                }

                if (currentInteractableObject.GetComponent<FPEInteractableAudioDiaryScript>())
                {
                    myHUDData.lookedAtAudioDiaryAutoPlay = currentInteractableObject
                        .GetComponent<FPEInteractableAudioDiaryScript>().AutomaticPlayback;
                }
            }
            else
            {
            ...
            if (currentPutbackObject != null)
            {
                if (currentHeldObject.GetComponent<FPEInteractableInventoryItemScript>())
                {
                    string myCustomPutbackString = currentHeldObject.GetComponent<FPEInteractableInventoryItemScript>()
                        .putBackString;
                    ...
                    if (myCustomPutbackString == "" || myCustomPutbackString == "<DEFAULT PUT BACK STRING>")
                    {
                        ...
                        myHUDData.lookedAtPickupPutbackString = currentHeldObject
                            .GetComponent<FPEInteractableInventoryItemScript>().ItemName;
                    }
                    else
                    {
                ...
                else
                {
                    myHUDData.lookedAtPickupPutbackString =
                        currentHeldObject.GetComponent<FPEInteractablePickupScript>().putBackString;
                }
            }
            else
            {
        ...
        public string[] FetchCurrentInteractionDebugData()
        {
            ...
            debugData[1] = (currentInteractableObject != null)
                ? "" + currentInteractableObject.GetComponent<FPEInteractableBaseScript>().getInteractionType()
                : "" + FPEInteractableBaseScript.eInteractionType.NULL_TYPE;
            ...
            debugData[5] = (currentHeldObject != null)
                ? "" + currentHeldObject.GetComponent<FPEInteractableBaseScript>().getInteractionType()
                : "" + FPEInteractableBaseScript.eInteractionType.NULL_TYPE;
        ...
        private void OnApplicationFocus(bool hasFocus)
        {
            ...
            if (hasFocus &&
                (currentInteractionState != eInteractionState.IN_MENU &&
                 currentInteractionState != eInteractionState.SUSPENDED) && Cursor.lockState != CursorLockMode.Locked)
            {
                Cursor.lockState = CursorLockMode.Locked;
            }
        }
        ...
        private void DockPlayer(FPEInteractableDockScript dock)
        {
            ...
            dock.dock();
            thePlayer.GetComponent<FPEFirstPersonController>().dockThePlayer(dock.DockTransform, dock.DockedViewLimits,
                dock.FocusTransform.position, dock.SmoothDock);
